...
until kubectl -n scylla get ingress tictactoe-ingress -o json | jq '.status.loadBalancer.ingress[0].ip' | tr -d '"' | grep -v null; do sleep 2; echo "Still no IP assigned"; done

IP=$(kubectl -n scylla get ingress tictactoe-ingress -o json | jq '.status.loadBalancer.ingress[0].ip' | tr -d '"')
