...
while getopts ":hnudrbp:c:z:v:s:t:" opt; do
	case $opt in
		h)  echo ""
		    ...
		    echo ""
		    echo "Usage:"
                    echo "-p   GCP project in which Scylla will be deployed (default: skilled-adapter-452). Usage: type '-p [MyProject]'"
                    echo "-z   Zone in which Scylla VMs will be deployed (default: us-east1-b). Usage: type '-z [Zone]'"
		    echo "-t   VM type (default: n1-standard-2). Example: to set n1-standard-8, type '-t8'"
...
if [ $SSD == "YES" ]; then
	...
	echo ""
	gcloud compute disks create "`whoami`-ansible-1-ssd-1" --size "$SSD_SIZE" --type "pd-ssd" --zone "$ZONE" &> /dev/null
	echo "### `whoami`-ansible-1-ssd-1 created"
        gcloud compute disks create "`whoami`-ansible-1-ssd-2" --size "$SSD_SIZE" --type "pd-ssd" --zone "$ZONE" &> /dev/null
        echo "### `whoami`-ansible-1-ssd-2 created"
        gcloud compute disks create "`whoami`-ansible-2-ssd-1" --size "$SSD_SIZE" --type "pd-ssd" --zone "$ZONE" &> /dev/null
        echo "### `whoami`-ansible-2-ssd-1 created"
        gcloud compute disks create "`whoami`-ansible-2-ssd-2" --size "$SSD_SIZE" --type "pd-ssd" --zone "$ZONE" &> /dev/null
        echo "### `whoami`-ansible-2-ssd-2 created"
        gcloud compute disks create "`whoami`-ansible-3-ssd-1" --size "$SSD_SIZE" --type "pd-ssd" --zone "$ZONE" &> /dev/null
        echo "### `whoami`-ansible-3-ssd-1 created"
        gcloud compute disks create "`whoami`-ansible-3-ssd-2" --size "$SSD_SIZE" --type "pd-ssd" --zone "$ZONE" &> /dev/null
        echo "### `whoami`-ansible-3-ssd-2 created"
...
# Create 3 CentOS7 VMs, boot disk 20GB, each VM with 2 SSD drives, 80GB storage
	...
	echo "### $VM_OS"
	echo ""
	gcloud compute --project "$PROJECT" instances create "`whoami`-scylla-$VER-ansible-1" --zone "$ZONE" --machine-type "n1-standard-$VM_TYPE" --network "default" --maintenance-policy "MIGRATE" --service-account "$PROJECT@appspot.gserviceaccount.com" --scopes "https://www.googleapis.com/auth/devstorage.read_only","https://www.googleapis.com/auth/logging.write","https://www.googleapis.com/auth/monitoring.write","https://www.googleapis.com/auth/servicecontrol","https://www.googleapis.com/auth/service.management.readonly","https://www.googleapis.com/auth/trace.append" --tags "http-server","https-server" --disk "name=`whoami`-ansible-1-ssd-1,device-name=`whoami`-ansible-1-ssd-1,mode=rw,boot=no,auto-delete=yes" --disk "name=`whoami`-ansible-1-ssd-2,device-name=`whoami`-ansible-1-ssd-2,mode=rw,boot=no,auto-delete=yes" $VM_OS --boot-disk-size "20" --boot-disk-type "pd-ssd" --boot-disk-device-name "`whoami`-ansible-1"
	gcloud compute --project "$PROJECT" instances create "`whoami`-scylla-$VER-ansible-2" --zone "$ZONE" --machine-type "n1-standard-$VM_TYPE" --network "default" --maintenance-policy "MIGRATE" --service-account "$PROJECT@appspot.gserviceaccount.com" --scopes "https://www.googleapis.com/auth/devstorage.read_only","https://www.googleapis.com/auth/logging.write","https://www.googleapis.com/auth/monitoring.write","https://www.googleapis.com/auth/servicecontrol","https://www.googleapis.com/auth/service.management.readonly","https://www.googleapis.com/auth/trace.append" --tags "http-server","https-server" --disk "name=`whoami`-ansible-2-ssd-1,device-name=`whoami`-ansible-2-ssd-1,mode=rw,boot=no,auto-delete=yes" --disk "name=`whoami`-ansible-2-ssd-2,device-name=`whoami`-ansible-2-ssd-2,mode=rw,boot=no,auto-delete=yes" $VM_OS --boot-disk-size "20" --boot-disk-type "pd-ssd" --boot-disk-device-name "`whoami`-ansible-2"
	gcloud compute --project "$PROJECT" instances create "`whoami`-scylla-$VER-ansible-3" --zone "$ZONE" --machine-type "n1-standard-$VM_TYPE" --network "default" --maintenance-policy "MIGRATE" --service-account "$PROJECT@appspot.gserviceaccount.com" --scopes "https://www.googleapis.com/auth/devstorage.read_only","https://www.googleapis.com/auth/logging.write","https://www.googleapis.com/auth/monitoring.write","https://www.googleapis.com/auth/servicecontrol","https://www.googleapis.com/auth/service.management.readonly","https://www.googleapis.com/auth/trace.append" --tags "http-server","https-server" --disk "name=`whoami`-ansible-3-ssd-1,device-name=`whoami`-ansible-3-ssd-1,mode=rw,boot=no,auto-delete=yes" --disk "name=`whoami`-ansible-3-ssd-2,device-name=`whoami`-ansible-3-ssd-2,mode=rw,boot=no,auto-delete=yes" $VM_OS --boot-disk-size "20" --boot-disk-type "pd-ssd" --boot-disk-device-name "`whoami`-ansible-3"

else

# Create 3 CentOS7 VMs, boot disk 20GB, each VM with 2 NVMe drives, 750GB storage
        echo ""
        echo "### Creating 3 VMs (n1-standard-$VM_TYPE) in '$ZONE' zone | 20GB boot disk | 375GB NVMe drive x $NVME_NUM"
	echo "### $VM_OS"
        echo ""
...
gcloud compute instances describe `whoami`-scylla-$VER-ansible-1 --zone "$ZONE" | grep -i networkip | cut -d ":" -f2 | awk '{$1=$1};1' >> servers.ini
gcloud compute instances describe `whoami`-scylla-$VER-ansible-2 --zone "$ZONE" | grep -i networkip | cut -d ":" -f2 | awk '{$1=$1};1' >> servers.ini
gcloud compute instances describe `whoami`-scylla-$VER-ansible-3 --zone "$ZONE" | grep -i networkip | cut -d ":" -f2 | awk '{$1=$1};1' >> servers.ini
...
NEW=`gcloud compute instances describe "$(whoami)"-scylla-$VER-ansible-1 --zone "$ZONE" | grep -i networkip | cut -d ":" -f2 | awk '{$1=$1};1'`
echo ""
...
if [ "$SSD" == "NO" ]; then
	...
	x=`echo "$DISK" | sed 's/,$//'`
	sed -i s~\/dev\/sdb,\/dev\/sdc~$x~g scylla_install_new.yml
fi
