...
func main() {
	...
	err = gocqlx.Query(session.Query(insertStmt), insertNames).BindStruct(r).BindStruct(r).ExecRelease()
	if err != nil {
		logger.Error("insert failed", zap.Error(err))
		return
	}
	logger.Info("Inserted Bob Johnson")

	// Delete the record
	deleteStmt, deleteNames := tbl.Delete()
	err = gocqlx.Query(session.Query(deleteStmt), deleteNames).BindStruct(Record{FirstName: "Bob", LastName: "Johnson"}).ExecRelease()
	if err != nil {
		logger.Error("delete failed", zap.Error(err))
		return
	}
	logger.Info("Deleted Bob Johnson")
} 