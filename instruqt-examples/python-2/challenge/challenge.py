...
class App:
    def __init__(self):
        ...
        self.delete_ps = self.session.prepare(
            query="DELETE FROM mutant_data WHERE first_name = ? and last_name = ?"
        )

        self.select_ps = self.session.prepare() # TODO: Implement the select statement

    def show_mutant_data(self):
        print("Data that we have in the catalog".center(50, "="))
        result = self.session.execute(self.select_ps)
        for row in result:
